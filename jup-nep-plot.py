...
# colors = [c.c97, c.c95, c.c93, c.c92, c.c91,]#, c.c99,]
...
for i in range(len(jups)):
    ...
    axs[1,0].plot(neps[i].age, neps[i].Bdyn, 
                color = colors[i], ls = '-.', label = labels_n[i])
    axs[0,1].plot(jups[i].age, jups[i].Bdip, 
                color = colors[i], ls = '-', label = labels_j[i])
    axs[1,1].plot(neps[i].age, neps[i].Bdip, 
                color = colors[i], ls = '-.', label = labels_n[i])
    
# Make nice
#axs[0].set_ylabel('Efficiency Factor', fontsize = 14)
axs[0,0].set_ylabel(r'$B_\mathrm{dyn}$ [G]', fontsize = 14)
axs[0,1].set_ylabel(r'$B_\mathrm{dip}$ [G]', fontsize = 14)
axs[1,0].set_ylabel(r'$B_\mathrm{dyn}$ [G]', fontsize = 14)
axs[1,1].set_ylabel(r'$B_\mathrm{dip}$ [G]', fontsize = 14)
