...
def hardB_doer_RCB(names):
    ...
    for name in names:
        ...
        for profile, i in zip(profiles, range(len(profiles))):
            ...
            # Get Rdyn, Schwartzchild
            R_dynamo_active = r[p.grada < p.gradr]
            try:
                Rdyn_end = R_dynamo_active[0] # it's the wrong way round
            except IndexError:
                # Save
...
def plotter(names, cols, labels, title, bigfirst = False):
    ...
    planets2 = hardB_doer_RCB(names)
    
    fig, axs = plt.subplots(1,2, tight_layout = True, sharex = True,
                           figsize = (8,4))
    
    custom_lines = []
    for planet, planet2, color, i in zip(planets, planets2, colors, range(len(planets))):
        
        axs[0].scatter(planet.age, planet.Bdyn, color = color)
        axs[1].scatter(planet2.age, planet2.Bdyn, color = color)
    ...
    # Make nice
    axs[0].set_ylabel('Dynamo Reynolds [G]', fontsize = 14)
    axs[1].set_ylabel('Dynamo RCB [G]', fontsize = 14)
