...
def plotter(names, cols, labels, title, 
    ...
    for planet, color, i in zip(planets, colors, range(len(planets))):
        ...
        axs[1].plot(planet.age, planet.avgrho , color = color) # cgs
        print(planet.q0)
        axs[2].plot(planet.age, planet.q0, color = color)
    ...
    axs[1].set_ylabel('Average density of dynamo region [g/cm$^3$]', fontsize = 14)
    axs[2].set_ylabel('Reference convective flux $q_0$ [W/m$^2$]', fontsize = 14)
    ...
    if met_hyd_lines:
        box=dict(facecolor = 'wheat', edgecolor = 'none', boxstyle = 'round', 
                 alpha = 0.7)
        axs[1].axhline(0.7, linestyle = '--', color = 'k')
        axs[1].text(5000, 0.65, 'Fluid Met. Hyd.', bbox = box)
        axs[1].axhline(1.15, linestyle = '-.', color = 'k')
        axs[1].text(5000, 1.1, 'Solid Met. Hyd.', bbox = box)
